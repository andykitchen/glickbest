...
def rank1_probs(key, ratings, num_samples=4000):
    """
    ...
    samples = jax.random.normal(key, shape=(num_players, num_samples))
    samples = scale*samples + loc
    counts = np.bincount(np.argmax(samples, axis=0), length=num_players)
...
def win_prob_bradley_terry(theta_1, theta_2, base=10., scale=400.):
    """
...
def update(ratings, i, j, s) -> None:
    ...
    ratings = ratings.at[i, 0].add(r_delta_1)
    ratings = ratings.at[i, 1].set(RD_prime_1)
    ratings = ratings.at[j, 0].add(r_delta_2)
    ratings = ratings.at[j, 1].set(RD_prime_2)
    return ratings


def next_match(key, ratings) -> tuple[int, int]:
    """
    pick next matchup

    :param ratings: (K, 2) ratings table, first column rating, second column ratings deviation (RD)
    """
    p_best = rank1_probs(key, ratings)
    i, j = top_k(p_best, k=2)
    return i, j


def next_match_thompson(key, ratings) -> tuple[int, int]:
    assert ratings.ndim == 2
    assert ratings.shape[1] == 2
    num_players = ratings.shape[0]
    loc = ratings[:,0]
    scale = ratings[:,1]
    sample = jax.random.normal(key, shape=(num_players,))
    sample = scale*sample + loc
    i, j = top_k(sample, k=2)
    return i, j


def test_glicko_known_values() -> None:
